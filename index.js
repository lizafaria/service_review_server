...
const uri = `mongodb+srv://${process.env.SECRET_USER}:${process.env.SECRET_PASSWORD}@cluster0.p8ggzbb.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.get("/homebooks", async (req, res) => {
      ...
      const result = await productitems.limit(3).toArray();
      res.send(result);
    });
    app.get("/homebooksall", async (req, res) => {
      const query = {};
      const productitems = await collection.find(query);
      const result = await productitems.toArray();
      res.send(result);
    });
